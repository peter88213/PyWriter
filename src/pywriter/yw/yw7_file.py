...
class Yw7File(File):
    ...
    def merge(self, source):
        ...
        if source.srtLocations:
            self.novel.srtLocations = source.srtLocations
            temploc = self.novel.locations
            self.novel.locations = {}
            for lcId in source.srtLocations:

                # Build a new self.novel.locations dictionary sorted like the source.
                self.novel.locations[lcId] = self.WE_CLASS()
                ...
                if source.locations[lcId].title:
                    ...
                    self.novel.locations[lcId].title = source.locations[lcId].title
                else:
                    self.novel.locations[lcId].title = temploc[lcId].title
                if source.locations[lcId].image is not None:
                    self.novel.locations[lcId].image = source.locations[lcId].image
                else:
                    self.novel.locations[lcId].desc = temploc[lcId].desc
                if source.locations[lcId].desc is not None:
                    self.novel.locations[lcId].desc = source.locations[lcId].desc
                else:
                    self.novel.locations[lcId].desc = temploc[lcId].desc
                if source.locations[lcId].aka is not None:
                    self.novel.locations[lcId].aka = source.locations[lcId].aka
                else:
                    self.novel.locations[lcId].aka = temploc[lcId].aka
                if source.locations[lcId].tags is not None:
                    self.novel.locations[lcId].tags = source.locations[lcId].tags
                else:
                    self.novel.locations[lcId].tags = temploc[lcId].tags
                for fieldName in self._LOC_KWVAR:
                    try:
                        self.novel.locations[lcId].kwVar[fieldName] = source.locations[lcId].kwVar[fieldName]
                    except:
                        self.novel.locations[lcId].kwVar[fieldName] = temploc[lcId].kwVar.get(fieldName, None)

        #--- Merge and re-order items.
        if source.srtItems:
            self.novel.srtItems = source.srtItems
            tempitm = self.novel.items
            self.novel.items = {}
            for itId in source.srtItems:

                # Build a new self.novel.items dictionary sorted like the source.
                self.novel.items[itId] = self.WE_CLASS()
                ...
                if source.items[itId].title:
                    # avoids deleting the title, if it is empty by accident
                    self.novel.items[itId].title = source.items[itId].title
                else:
                    self.novel.items[itId].title = tempitm[itId].title
                if source.items[itId].image is not None:
                    self.novel.items[itId].image = source.items[itId].image
                else:
                    self.novel.items[itId].image = tempitm[itId].image
                if source.items[itId].desc is not None:
                    self.novel.items[itId].desc = source.items[itId].desc
                else:
                    self.novel.items[itId].desc = tempitm[itId].desc
                if source.items[itId].aka is not None:
                    self.novel.items[itId].aka = source.items[itId].aka
                else:
                    self.novel.items[itId].aka = tempitm[itId].aka
                if source.items[itId].tags is not None:
                    self.novel.items[itId].tags = source.items[itId].tags
                else:
                    self.novel.items[itId].tags = tempitm[itId].tags
                for fieldName in self._ITM_KWVAR:
                    try:
                        self.novel.items[itId].kwVar[fieldName] = source.items[itId].kwVar[fieldName]
                    except:
                        self.novel.items[itId].kwVar[fieldName] = tempitm[itId].kwVar.get(fieldName, None)

        #--- Merge and re-order characters.
        if source.srtCharacters:
            self.novel.srtCharacters = source.srtCharacters
            tempchr = self.novel.characters
            self.novel.characters = {}
            for crId in source.srtCharacters:

                # Build a new self.novel.characters dictionary sorted like the source.
                self.novel.characters[crId] = self.CHARACTER_CLASS()
                ...
                if source.characters[crId].title:
                    # avoids deleting the title, if it is empty by accident
                    self.novel.characters[crId].title = source.characters[crId].title
                else:
                    self.novel.characters[crId].title = tempchr[crId].title
                if source.characters[crId].image is not None:
                    self.novel.characters[crId].image = source.characters[crId].image
                else:
                    self.novel.characters[crId].image = tempchr[crId].image
                if source.characters[crId].desc is not None:
                    self.novel.characters[crId].desc = source.characters[crId].desc
                else:
                    self.novel.characters[crId].desc = tempchr[crId].desc
                if source.characters[crId].aka is not None:
                    self.novel.characters[crId].aka = source.characters[crId].aka
                else:
                    self.novel.characters[crId].aka = tempchr[crId].aka
                if source.characters[crId].tags is not None:
                    self.novel.characters[crId].tags = source.characters[crId].tags
                else:
                    self.novel.characters[crId].tags = tempchr[crId].tags
                if source.characters[crId].notes is not None:
                    self.novel.characters[crId].notes = source.characters[crId].notes
                else:
                    self.novel.characters[crId].notes = tempchr[crId].notes
                if source.characters[crId].bio is not None:
                    self.novel.characters[crId].bio = source.characters[crId].bio
                else:
                    self.novel.characters[crId].bio = tempchr[crId].bio
                if source.characters[crId].goals is not None:
                    self.novel.characters[crId].goals = source.characters[crId].goals
                else:
                    self.novel.characters[crId].goals = tempchr[crId].goals
                if source.characters[crId].fullName is not None:
                    self.novel.characters[crId].fullName = source.characters[crId].fullName
                else:
                    self.novel.characters[crId].fullName = tempchr[crId].fullName
                if source.characters[crId].isMajor is not None:
                    self.novel.characters[crId].isMajor = source.characters[crId].isMajor
                else:
                    self.novel.characters[crId].isMajor = tempchr[crId].isMajor
                for fieldName in self._CRT_KWVAR:
                    try:
                        self.novel.characters[crId].kwVar[fieldName] = source.characters[crId].kwVar[fieldName]
                    except:
                        self.novel.characters[crId].kwVar[fieldName] = tempchr[crId].kwVar.get(fieldName, None)

        #--- Merge and re-order projectNotes.
        if source.srtPrjNotes:
            self.novel.srtPrjNotes = source.srtPrjNotes
            tempPrjn = self.novel.projectNotes
            self.novel.projectNotes = {}
            for pnId in source.srtPrjNotes:

                # Build a new self.novel.projectNotes dictionary sorted like the source.
                self.novel.projectNotes[pnId] = self.PN_CLASS()
                ...
                if source.projectNotes[pnId].title:
                    # avoids deleting the title, if it is empty by accident
                    self.novel.projectNotes[pnId].title = source.projectNotes[pnId].title
                else:
                    self.novel.projectNotes[pnId].title = tempPrjn[pnId].title

                if source.projectNotes[pnId].desc is not None:
                    self.novel.projectNotes[pnId].desc = source.projectNotes[pnId].desc
                else:
                    self.novel.projectNotes[pnId].desc = tempPrjn[pnId].desc

                for fieldName in self._prn_KWVAR:
                    try:
                        self.novel.projectNotes[pnId].kwVar[fieldName] = source.projectNotes[pnId].kwVar[fieldName]
                    except:
                        self.novel.projectNotes[pnId].kwVar[fieldName] = tempPrjn[pnId].kwVar.get(fieldName, None)
        ...
        for scId in source.scenes:
            ...
            if source.scenes[scId].title:
                # avoids deleting the title, if it is empty by accident
                self.novel.scenes[scId].title = source.scenes[scId].title
            ...
            if source.scenes[scId].sceneContent is not None:
                self.novel.scenes[scId].sceneContent = source.scenes[scId].sceneContent
            ...
            if source.scenes[scId].status is not None:
                self.novel.scenes[scId].status = source.scenes[scId].status

            if source.scenes[scId].notes is not None:
                self.novel.scenes[scId].notes = source.scenes[scId].notes

            if source.scenes[scId].tags is not None:
                self.novel.scenes[scId].tags = source.scenes[scId].tags

            if source.scenes[scId].field1 is not None:
                self.novel.scenes[scId].field1 = source.scenes[scId].field1

            if source.scenes[scId].field2 is not None:
                self.novel.scenes[scId].field2 = source.scenes[scId].field2

            if source.scenes[scId].field3 is not None:
                self.novel.scenes[scId].field3 = source.scenes[scId].field3

            if source.scenes[scId].field4 is not None:
                self.novel.scenes[scId].field4 = source.scenes[scId].field4

            if source.scenes[scId].appendToPrev is not None:
                self.novel.scenes[scId].appendToPrev = source.scenes[scId].appendToPrev

            if source.scenes[scId].date or source.scenes[scId].time:
                if source.scenes[scId].date is not None:
                    self.novel.scenes[scId].date = source.scenes[scId].date
                if source.scenes[scId].time is not None:
                    self.novel.scenes[scId].time = source.scenes[scId].time
            ...
            if source.scenes[scId].minute is not None:
                self.novel.scenes[scId].minute = source.scenes[scId].minute

            if source.scenes[scId].hour is not None:
                self.novel.scenes[scId].hour = source.scenes[scId].hour

            if source.scenes[scId].day is not None:
                self.novel.scenes[scId].day = source.scenes[scId].day

            if source.scenes[scId].lastsMinutes is not None:
                self.novel.scenes[scId].lastsMinutes = source.scenes[scId].lastsMinutes

            if source.scenes[scId].lastsHours is not None:
                self.novel.scenes[scId].lastsHours = source.scenes[scId].lastsHours

            if source.scenes[scId].lastsDays is not None:
                self.novel.scenes[scId].lastsDays = source.scenes[scId].lastsDays

            if source.scenes[scId].isReactionScene is not None:
                self.novel.scenes[scId].isReactionScene = source.scenes[scId].isReactionScene

            if source.scenes[scId].isSubPlot is not None:
                self.novel.scenes[scId].isSubPlot = source.scenes[scId].isSubPlot

            if source.scenes[scId].goal is not None:
                self.novel.scenes[scId].goal = source.scenes[scId].goal

            if source.scenes[scId].conflict is not None:
                self.novel.scenes[scId].conflict = source.scenes[scId].conflict

            if source.scenes[scId].outcome is not None:
                self.novel.scenes[scId].outcome = source.scenes[scId].outcome
            ...
            for fieldName in self._SCN_KWVAR:
                try:
                    self.novel.scenes[scId].kwVar[fieldName] = source.scenes[scId].kwVar[fieldName]
                except:
                    pass
        ...
        for chId in source.chapters:
            ...
            if source.chapters[chId].title:
                # avoids deleting the title, if it is empty by accident
                self.novel.chapters[chId].title = source.chapters[chId].title

            if source.chapters[chId].desc is not None:
                self.novel.chapters[chId].desc = source.chapters[chId].desc

            if source.chapters[chId].chLevel is not None:
                self.novel.chapters[chId].chLevel = source.chapters[chId].chLevel

            if source.chapters[chId].chType is not None:
                self.novel.chapters[chId].chType = source.chapters[chId].chType

            if source.chapters[chId].suppressChapterTitle is not None:
                self.novel.chapters[chId].suppressChapterTitle = source.chapters[chId].suppressChapterTitle

            if source.chapters[chId].suppressChapterBreak is not None:
                self.novel.chapters[chId].suppressChapterBreak = source.chapters[chId].suppressChapterBreak

            if source.chapters[chId].isTrash is not None:
                self.novel.chapters[chId].isTrash = source.chapters[chId].isTrash

            for fieldName in self._CHP_KWVAR:
                try:
                    self.novel.chapters[chId].kwVar[fieldName] = source.chapters[chId].kwVar[fieldName]
                except:
                    pass
            ...
            self.novel.chapters[chId].srtScenes = srtScenes

            # Add new or moved scenes to the scene list.
            merge_lists(source.chapters[chId].srtScenes, self.novel.chapters[chId].srtScenes)

        #--- Merge project attributes.
        if source.title:
            # avoids deleting the title, if it is empty by accident
            self.novel.title = source.title

        if source.desc is not None:
            self.novel.desc = source.desc

        if source.authorName is not None:
            self.novel.authorName = source.authorName

        if source.authorBio is not None:
            self.novel.authorBio = source.authorBio

        if source.fieldTitle1 is not None:
            self.novel.fieldTitle1 = source.fieldTitle1

        if source.fieldTitle2 is not None:
            self.novel.fieldTitle2 = source.fieldTitle2

        if source.fieldTitle3 is not None:
            self.novel.fieldTitle3 = source.fieldTitle3

        if source.fieldTitle4 is not None:
            self.novel.fieldTitle4 = source.fieldTitle4

        if source.languageCode is not None:
            self.novel.languageCode = source.languageCode

        if source.countryCode is not None:
            self.novel.countryCode = source.countryCode

        if source.languages is not None:
            self.novel.languages = source.languages

        for fieldName in self._PRJ_KWVAR:
            try:
                self.novel.kwVar[fieldName] = source.kwVar[fieldName]
            except:
                pass
        ...
        merge_lists(source.srtChapters, self.novel.srtChapters)
