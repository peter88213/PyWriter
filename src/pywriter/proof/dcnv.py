...
class DCnv():
    ...
    def __init__(self, yw7Path, documentPath):
        ...
        self.officeFile = OfficeFile(self.documentPath)
    ...
    def yw7_to_document(self):
        ...
        if self.officeFile.file_exists():

            if not self.confirm_overwrite(self.documentPath):
                return('Program abort by user.')

        return(self.officeFile.write(self.yw7File))
    ...
    def document_to_yw7(self):
        ...
        if self.officeFile.filePath is None:
            return('ERROR: "' + self.documentPath + '" is not an Office file.')

        if not self.officeFile.file_exists():
            return('ERROR: "' + self.documentPath + '" not found.')

        message = self.officeFile.read()
        ...
        prjStructure = self.officeFile.get_structure()
        ...
        return(self.yw7File.write(self.officeFile))
